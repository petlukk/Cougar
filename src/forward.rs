...
mod tests {
    ...
    #[test]
    fn argmax_all_equal() {
        // All equal: Rust's max_by returns the last equal element, so last index wins
        let v = [5.0f32, 5.0, 5.0];
        let idx = argmax(&v) as usize;
        // Any index is valid; just confirm the value at the returned index equals the max
    ...
    fn sample_high_temperature_still_valid() {
        // With very high temperature scaling still produces a valid index
    ...
    fn rope_freqs_pos0_cos1_sin0() {
        ...
        // At pos=0, angle = 0 for all i, so cos=1 and sin=0
        for i in 0..head_dim / 2 {
            let cos = freqs[2 * i];
            let sin = freqs[2 * i + 1];
    ...
    fn rope_freqs_output_length() {
        ...
        // Should have written head_dim values (head_dim/2 pairs of cos,sin)
        assert_eq!(freqs.len(), head_dim);
    }

    #[test]
    fn rope_freqs_pos1_first_pair() {
        // At pos=1, i=0: angle = 1 / theta^0 = 1.0
        let head_dim = 4usize;
        let theta = 10000.0f32;
        let mut freqs = vec![0.0f32; head_dim];
        build_rope_freqs(&mut freqs, head_dim, 1, theta);

        let angle0 = 1.0f32 / theta.powf(0.0); // = 1.0
    ...
    fn apply_rope_pos0_identity() {
        // pos=0 → all cos=1, sin=0 → data unchanged
        let head_dim = 4usize;
        let n_heads = 2usize;
        let mut freqs = vec![0.0f32; head_dim];
        build_rope_freqs(&mut freqs, head_dim, 0, 10000.0);
    ...
    fn apply_rope_known_rotation() {
        // Single head, head_dim=2: one rotation pair
        // freqs[0]=cos(π/2)=0, freqs[1]=sin(π/2)=1
        // data = [r, im] = [1.0, 0.0]
        // expected: r' = r*cos - im*sin = 1*0 - 0*1 = 0.0
        //           im' = r*sin + im*cos = 1*1 + 0*0 = 1.0
        let head_dim = 2usize;
        let n_heads = 1usize;
        let angle = std::f32::consts::FRAC_PI_2; // π/2
    ...
    fn apply_rope_multi_head_consistent() {
        // Two identical heads should produce identical rotated outputs
        let head_dim = 4usize;
        let n_heads = 2usize;
        let mut freqs = vec![0.0f32; head_dim];
        build_rope_freqs(&mut freqs, head_dim, 5, 10000.0);

        // Both heads have same initial values
        let mut data: Vec<f32> = (0..head_dim * n_heads).map(|i| (i % head_dim) as f32).collect();
        apply_rope(&mut data, &freqs, head_dim, n_heads);

        // Head 0 and head 1 should be identical since they started identical
        for i in 0..head_dim {
            assert!(
