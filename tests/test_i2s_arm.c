// test_i2s_arm.c — Validates ARM NEON kernel against scalar reference
//
// Same tests as test_i2s.c but for the ARM kernel which uses int8_t* weights
// (signed × signed via vdot_i32) instead of uint8_t* (unsigned × signed via maddubs_i32).
//
// Build (on Pi 5):
//   gcc -O2 tests/test_i2s_arm.c -Lbuild/lib -lbitnet_i2s_arm -o build/test_i2s_arm -lm
//
// Run:
//   LD_LIBRARY_PATH=build/lib ./build/test_i2s_arm
...
// FFI declarations — ARM kernel uses int8_t* for weights (signed × signed)
extern int32_t i2_dot_i8(const int8_t *weights, const int8_t *activations, int32_t n);
...
// Scalar reference: same math as x86 reference, uses uint8_t internally
// (the packed bits are identical regardless of pointer signedness)
...
static void pack_ternary(const int8_t *ternary, uint8_t *packed, int n) {
    ...
    for (int blk = 0; blk < n_blocks; blk++) {
        for (int i = 0; i < 32; i++) {
            int base = blk * 128;
            uint8_t g0 = (uint8_t)(ternary[base + i]      + 1);
...
static int test_single_row(int n) {
    ...
    int32_t got      = i2_dot_i8((const int8_t *)packed, act, n);
...
static int test_4row(int n) {
    ...
    int32_t got[4];
    i2_dot_i8_4row(
        (const int8_t *)packed[0], (const int8_t *)packed[1],
        (const int8_t *)packed[2], (const int8_t *)packed[3],
        act, got, n
    );
...
static double bench_single_row(int n, int iters) {
    ...
    for (int i = 0; i < 100; i++) sink += i2_dot_i8((const int8_t *)packed, act, n);

    struct timespec t0, t1;
    clock_gettime(CLOCK_MONOTONIC, &t0);
    for (int i = 0; i < iters; i++) {
        sink += i2_dot_i8((const int8_t *)packed, act, n);
    }
...
int main(void) {
    ...
    printf("=== BitNet I2_S ARM NEON kernel tests ===\n\n");
    ...
    if (pass == total) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            double gops = (double)bench_sizes[i] / ns;
